...
namespace RuntimeGizmos
{
	...
	{
		public TransformWizard tw;  //QRAY TRANSFORM WIZARD.
		...
		void Update()
		{
			...
			if(Input.GetKey(KeyCode.LeftControl) && Input.GetKeyDown(KeyCode.D)) {  //Duplicates object and selects it.
				target = Instantiate(target.gameObject).transform;
				tw.SelectObject(target.gameObject);
			}
			TransformSelected();
		}
		...
		void SetType(string t) {
			if(t == "Move") type = TransformType.Move;
			if(t == "Rotate") type = TransformType.Rotate;
			if(t == "Scale") type = TransformType.Scale;
		}
		...
		{
			if(selectedAxis != Axis.None && Input.GetMouseButtonDown(0) && !UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject())
			{
		...
		void GetTarget()
		{
			if(selectedAxis == Axis.None && Input.GetMouseButtonDown(0) && !UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject())
			{
				...
				if(Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo))
				{
					...
					tw.SelectObject(hitInfo.transform.gameObject);
				}else{
					target = null;
					twPanel.SetActive(false);
				}
